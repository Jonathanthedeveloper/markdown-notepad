...
>(({ className, ...props }, ref) => {
  ...
  return (
    <Tooltip>
      <TooltipTrigger render={<DropdownMenu />}><DropdownMenuTrigger render={<Button variant="ghost" size="sm" className={cn(
        "h-8 w-max gap-1 px-3 font-normal",
        editor?.isActive("heading") && "bg-accent",
        className
      )} ref={ref} {...props} />}>{activeLevel ? `H${activeLevel}` : "Normal"}<ChevronDown className="h-4 w-4" /></DropdownMenuTrigger><DropdownMenuContent align="start">
          <DropdownMenuItem
            onClick={() => editor?.chain().focus().setParagraph().run()}
            className={cn(
        ...
        </DropdownMenuContent></TooltipTrigger>
      <TooltipContent>
        <span>Headings</span>
      </TooltipContent>
    </Tooltip>
