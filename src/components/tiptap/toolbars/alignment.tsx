...
export const AlignmentToolbar = () => {
  ...
  return (
    <DropdownMenu>
      <Tooltip>
        <TooltipTrigger render={<DropdownMenuTrigger disabled={isDisabled} render={<Button variant="ghost" size="sm" className="h-8 w-max font-normal" />} />}><span className="mr-2">
          {alignmentOptions[findIndex(currentTextAlign())]?.icon}
        </span>{alignmentOptions[findIndex(currentTextAlign())]?.name}<ChevronDown className="ml-2 h-4 w-4" /></TooltipTrigger>
