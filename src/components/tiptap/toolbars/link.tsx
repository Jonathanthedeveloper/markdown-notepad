...
const LinkToolbar = React.forwardRef<HTMLButtonElement, Parameters<typeof Button>[0]>((
	{ className, ...props }, ref) => {
	...
	return (
		<Popover>
			<Tooltip>
				<TooltipTrigger render={<PopoverTrigger disabled={!editor?.can().chain().setLink({ href: "" }).run()} render={<Button variant="ghost" size="sm" className={cn(
					"h-8 w-max px-3 font-normal",
					editor?.isActive("link") && "bg-accent",
					className,
				)} ref={ref} {...props} />} />}><p className="mr-2 text-base">↗</p><p className={"underline decoration-gray-7 underline-offset-4"}>
						Link
					</p></TooltipTrigger>
				<TooltipContent>
					<span>Link</span>
